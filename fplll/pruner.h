...
/**
  ...
  * The function is templated by a Floating Point Type, which are used for internal computations.
  *
  * Calling M the metric function (Success proba or expectation of number of results),
  * C the cost function of enumeration, P the preproc_cost, this function tries to 
  * provide a pruning vector p essentially minimizing (C(p) + P) / M(p).
  * The pruning vector is expressed as relative (squared) length with respect to
  * the enumeration radius, i.e. the entries of p are in the interval (0,1), and
  * are always decreasing.
  * 
  * Small amendents are made to also take account for the desired target value of the metric
  * after repeating several enumeration. In particular the chosen pruning value should
  * not excess the target, and a bit more enumeration may be chosen if it get is to
  * the the desired result without any retrial.
  * 
  * The cost C of enumeration under the gaussian heuristic, and depends on gso_r.
  * The function S is define by the metric parameter (and depend on gso_r for EXPECTED_SOLUTIONS)
  *
  ...
  *
  * If several algortihm are activated, they will all be ran, using the output of the previous
  * as there starting point.
  * 
  * Greedy shoud be very fast, and essentially tries to produce pruning parameters generating a 
  * flat enumeration tree. It is meant to quickly provide pruning parameters, say up to block 40.
  * It is also used as a decent starting point for the gradient descent to be quicker.
  * It can be argued that this strategy is a factor O(n) away from optimal, probably less in
  * practice. 
  *
  * The gradient descent is rather standard. The gradient is computed numerically, and then
  * several steps are made keeping the same direction until a local minima in this direction 
  * is found (the rational is that computing the gradient is 2n times more expensive than test
  * a point).
  * It is to be expected that this method suffers from convergence and numerical instability especially
  * due to numerical differentiation. It remains rather fast and seems to properly handle
  * basis up to dimension LLL reduced-basis up to dim ~70, and more if the basis is strongly
  * reduced.
  *
  * The Nelder-Mead algorithm is a descent algorithm praised for its robustness. It's pretty
  * cool, you should really read about it: https://en.wikipedia.org/wiki/Nelder%E2%80%93Mead_method .
  * In brief, it maintains a simplex of points and each step consist in improving the worst
  * of them by reflexion expansion or contraction, or shrinking the all simplex if the above failed.
  * This method is somehow aware not only of the gradient, but also the curvature of the optimized
  ...
  * @param metric metric is to be optimized : PRUNER_METRIC_PROBABILITY_OF_SHORTEST or PRUNER_METRIC_EXPECTED_SOLUTIONS 
  * @param flags complementary parameters : PRUNER_CVP  PRUNER_START_FROM_INPUT  PRUNER_GRADIENT  PRUNER_NELDER_MEAD  PRUNER_VERBOSE
...
/**
  ...
  * @param enumeration_radius radius of the desired enumeration
  * @param preproc_cost cost of preprocessing (i.e. additive cost for a retrying an enumeration)
  * @param gso_rs vector of vector Gram-Schmidt length (squared) of the enumerated block over several bases
  * @param target desired target success probability/expected solutions after all retrial. 
  * @param metric metric is to be optimized : PRUNER_METRIC_PROBABILITY_OF_SHORTEST or PRUNER_METRIC_EXPECTED_SOLUTIONS 
  * @param flags complementary parameters : PRUNER_CVP  PRUNER_START_FROM_INPUT  PRUNER_GRADIENT  PRUNER_NELDER_MEAD  PRUNER_VERBOSE
  *
...
/**
  ...
  * @param pruning A pruning object. Only uses the coefficient field. (coefficients must start with two 1s, be decreasing and in the interval (0,1))
  * @return probability 
*/
...
/**
  * @brief Probability that a pruned enumeration indeed returns the shortest vector.
  * @param pr A vector of pruning bounds (coefficients must start with two 1s, be decreasing and in the interval (0,1))
  * @return probability 
*/
...
/**
  ...
  * strategy of [Gama Nguyen Regev 2010].
  *
  ...
  * allow volume estimation by symbolic integration (see [GNR10,Chen13])
  *
  * naming conventions:
...
*/
  Pruner(const int n)
      : n(n), metric(PRUNER_METRIC_PROBABILITY_OF_SHORTEST), flags(0)
  {
...
/**
  * Algorithmic details are given for the wrapping function prune().
  * @brief Constructor for Pruner with multiple basis (averaging the cost function over all of them)
  * @param enumeration_radius radius of the desired enumeration
  * @param preproc_cost cost of preprocessing (i.e. additive cost for a retrying an enumeration)
  * @param gso_rs vector of vector Gram-Schmidt length (squared) of the enumerated block over several bases
  * @param target desired target success probability/expected solutions after all retrial. 
...
*/
  ...
  */
  ...
  /** 
  Compute the cost of r enumeration and (r-1) preprocessing, where r 
...
private:
  ...
  FT target;
  PrunerMetric metric;
  bool shape_loaded = false; ///< Keep track of whether a basis was loaded. Only allows single_enum_cost() with metric PROBABILITY_OF_SHORTEST if not
  int flags;
  int n;      ///< Dimension of the block to be enumerated
  int d;      ///< Half dimension: most function only use half dimension, as our Pruner implcitly enforce parity
  ...
  */
  ...
  /**
  Enforce that pruning coeffient terminate with a 1, are decreasing, 
  and are not too close to 0 (for numerical stability).
  ...
  */
  ...
  /**
  @brief Compute the relative volume of a cylinder intersection to the unit sphere (of a given subdimension)
...
template <class FT> bool Pruner<FT>::tabulated_values_imported = false; ///< (static) tabulated value not loaded yet
