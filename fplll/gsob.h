...
/**
 * MatGSOb provides an interface for performing elementary operations on a basis
...
template <class ZT, class FT> class MatGSOb
{
public:
  /**
   * Constructor.
  ...
  MatGSOb(Matrix<ZT> &arg_u, Matrix<ZT> &arg_uinv_t, int flags)
  ...
  */
  ...
  const Matrix<ZT> &get_g_matrix() { 
    if (gptr == nullptr) { cerr << "Error: gptr == nullpointer.\n"; exit(1); }
    return *gptr; 
  }
...
template <class ZT, class FT> inline MatGSOb<ZT,FT>::~MatGSOb()
{
  //delete gptr;
}

template <class ZT, class FT> inline void MatGSOb<ZT,FT>::symmetrize_g()
{
...
template <class ZT, class FT> inline void MatGSOb<ZT,FT>::print_mu_r_g(ostream &os)
{
  os << "mu = " << endl;
  mu.print(os);
  os << endl << "r = " << endl;
...
template <class ZT, class FT> inline ZT &MatGSOb<ZT,FT>::sym_g(int i, int j) { 
  if (gptr == nullptr) {
...
/*
template <class ZT, class FT> inline FT &MatGSOb<ZT, FT>::get_gram(FT &f, int i, int j)
{
...
template <class ZT, class FT> inline const FT &MatGSOb<ZT, FT>::get_mu_exp(int i, int j, long &expo)
{
...
template <class ZT, class FT> inline const FT &MatGSOb<ZT, FT>::get_mu_exp(int i, int j)
{
  FPLLL_DEBUG_CHECK(i >= 0 && i < n_known_rows && j >= 0 && j < i && j < gso_valid_cols[i] &&
                    !in_row_op_range(i));
  return mu(i, j);
...
template <class ZT, class FT> inline FT &MatGSOb<ZT, FT>::get_mu(FT &f, int i, int j)
{
  FPLLL_DEBUG_CHECK(i >= 0 && i < n_known_rows && j >= 0 && j < i && j < gso_valid_cols[i] &&
                    !in_row_op_range(i));
  f = mu(i, j);
...
template <class ZT, class FT> inline const FT &MatGSOb<ZT, FT>::get_r_exp(int i, int j, long &expo)
{
...
template <class ZT, class FT> inline const FT &MatGSOb<ZT, FT>::get_r_exp(int i, int j)
{
  FPLLL_DEBUG_CHECK(i >= 0 && i < n_known_rows && j >= 0 && j < gso_valid_cols[i] &&
                    !in_row_op_range(i));
  return r(i, j);
...
template <class ZT, class FT> inline FT &MatGSOb<ZT, FT>::get_r(FT &f, int i, int j)
{
  FPLLL_DEBUG_CHECK(i >= 0 && i < n_known_rows && j >= 0 && j < gso_valid_cols[i] &&
                    !in_row_op_range(i));
  f = r(i, j);
...
template <class ZT, class FT> inline bool MatGSOb<ZT, FT>::update_gso_row(int i)
{
  return update_gso_row(i, i);
}

template <class ZT, class FT> inline void MatGSOb<ZT, FT>::set_r(int i, int j, FT &f)
{
...
template <class ZT, class FT> inline void MatGSOb<ZT, FT>::row_addmul(int i, int j, const FT &x)
{
  row_addmul_we(i, j, x, 0);
}

template <class ZT, class FT> inline void MatGSOb<ZT, FT>::create_row() { create_rows(1); }

template <class ZT, class FT> inline void MatGSOb<ZT, FT>::remove_last_row() { remove_last_rows(1); }

template <class ZT, class FT> inline void MatGSOb<ZT, FT>::discover_all_rows()
{
  while (n_known_rows < d)
    discover_row();
}

template <class ZT, class FT> inline bool MatGSOb<ZT, FT>::update_gso()
{
  for (int i = 0; i < d; i++)
  {
    if (!update_gso_row(i))
...
#ifdef DEBUG
template <class ZT, class FT> inline void MatGSOb<ZT, FT>::row_op_begin(int first, int last)
{
...
inline void MatGSOb<ZT, FT>::dump_mu_d(double *mu, int offset, int block_size)
{
  FT e;
  if (block_size <= 0)
  {
...
template <class ZT, class FT>
inline void MatGSOb<ZT, FT>::dump_mu_d(vector<double> mu, int offset, int block_size)
{
  FT e;
  if (block_size <= 0)
  {
...
template <class ZT, class FT>
inline void MatGSOb<ZT, FT>::dump_r_d(double *r, int offset, int block_size)
{
  FT e;
  if (block_size <= 0)
  {
...
template <class ZT, class FT>
inline void MatGSOb<ZT, FT>::dump_r_d(vector<double> r, int offset, int block_size)
{
  FT e;
  if (block_size <= 0)
  {
