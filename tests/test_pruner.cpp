...
// XXX = .7 ---> 0.921775209040006
  ...
  int test_relative_volume(){
    ...
    for (int i = 0; i < Nbis; ++i)
      {rs[i] = 1.;}
    pru.load_basis_shape(Nbis, rs);
    double proba, error;

    for (int i = 0; i < Nbis/2; ++i)
      { pr[i] = 1; pr[i+(Nbis/2)] = .3;}
...
void set_up_gso_norms(double* gso_sq_norms){
  for (int i = 0; i < N; ++i)
  {
    // A bit more than LLL reduced
    gso_sq_norms[i] = pow(1.06, - i);
  }  
}
...
int test_prepruned(){
  ...
  Pruner<FT> pru;
  double gso_sq_norms[N];
  set_up_gso_norms(gso_sq_norms);
  pru.load_basis_shape(N, gso_sq_norms);
